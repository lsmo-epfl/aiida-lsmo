"""IsothermInflection work chain: 2nd protocol"""
...
def get_output_parameters(pressures, geom_out, widom_out, **gcmc_dict):
    ...
    if out_dict['is_porous'] and widom_out:
        ...
        for i in range(len(pressures)):
            for inp, key_ext in [('mid', ""), ('dil', "_from_dil"), ('sat', "_from_sat")]:
...
class IsothermInflection2WorkChain(WorkChain):
    ...
    def define(cls, spec):
        ...
        spec.outline(
            ...
            if_(cls.should_run_widom)(  # if is porous
                ...
                cls.run_mid_dens_nvt,  # compute mid density and run NVT
                cls.run_gcmc_from_mid,  # run GCMC from mid-density NVT
            ),
    ...
    def _update_param_for_nvt(self, number_of_molecules):
        ...
        param["Component"][self.ctx.molecule['name']].update({
            "SwapProbability": 0.0,
            "CreateNumberOfMolecules": int(number_of_molecules),
        })
        return param
    ...
    def run_raspa_widom_and_sat(self):
        ...
        self.ctx.raspa_param = self._update_param_for_nvt(nmolecules)
        self.ctx.inp['raspa']['parameters'] = Dict(dict=self.ctx.raspa_param)
    ...
    def run_mid_dens_nvt(self):
        """For each pressure point, run a NVT calculation at half the molecules obtained in the previous GCMC calculations."""

        del self.ctx.inp['raspa']['retrieved_parent_folder']

        for i, pressure in enumerate(self.ctx.pressures):
            # Update labels
            self.ctx.inp['metadata']['label'] = "RaspaNVT_{}".format(i + 1)
            self.ctx.inp['metadata']['call_link_label'] = "run_raspa_nvt_{}".format(i + 1)

            # Update parameters Dict
            mid_dens_molecules = self._get_mid_dens_molecules(self.ctx['raspa_gcmc_dil_{}'.format(i + 1)],
                                                              self.ctx['raspa_gcmc_sat_{}'.format(i + 1)])
            self.ctx.raspa_param = self._update_param_for_nvt(mid_dens_molecules)
            self.ctx.inp['raspa']['parameters'] = Dict(dict=self.ctx.raspa_param)

            # Create the calculation process, launch it and update pressure index
            running = self.submit(RaspaBaseWorkChain, **self.ctx.inp)
            self.report("Running Raspa NVT mid {} with {} molecules".format(i + 1, mid_dens_molecules))

            key_ctx = 'raspa_nvt_mid_{}'.format(i + 1)
            self.to_context(**{key_ctx: running})

    def run_gcmc_from_mid(self):
        """For each pressure point, run GCMC from mid-density NVT final configuration."""

        self.ctx.raspa_param = self._update_param_for_gcmc()

        for i, pressure in enumerate(self.ctx.pressures):
            # Update labels
            self.ctx.inp['metadata']['label'] = "RaspaGCMC_mid_{}".format(i + 1)
            self.ctx.inp['metadata']['call_link_label'] = "run_raspa_gcmc_mid_{}".format(i + 1)

            self.ctx.raspa_param["System"]["framework_1"]['ExternalPressure'] = self.ctx.pressures[i] * 1e5

            # Update parameters Dict
            self.ctx.inp['raspa']['parameters'] = Dict(dict=self.ctx.raspa_param)
            self.ctx.inp['raspa']['retrieved_parent_folder'] = self.ctx['raspa_nvt_mid_{}'.format(i +
                                                                                                  1)].outputs.retrieved

            # Create the calculation process, launch it and update pressure index
            running = self.submit(RaspaBaseWorkChain, **self.ctx.inp)
            self.report("Running Raspa GCMC mid {}".format(i + 1))

            key_ctx = 'raspa_gcmc_mid_{}'.format(i + 1)
            self.to_context(**{key_ctx: running})
