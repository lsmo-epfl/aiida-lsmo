...
def render_ff_mixing_def(ff_data, params):
    ...
    output.append("# general rule tailcorrections")
    output.append(['no', 'yes'][params['tailcorrections']])
...
def render_ff_def(ff_data, params):
    ...
    if params['separate_interactions']:
        ff_list = []
        for molecule, ff_name in params["ff_molecules"].items():
            for atom_type, val in ff_data[molecule][ff_name]['atom_types'].items():
                ff_pot = val['force_field']
                if ff_pot[0].lower() != "none":
                    ff_list.append([atom_type] + ff_pot)
        if not all([x[1].lower() == "lennard-jones" for x in ff_list]):  #TODO: aiida exit #pylint: disable=fixme
            sys.exit('WARNING: separate_interactions not working for potential not lennard-jones')
        ff_mix = []
        for i, ffi in enumerate(ff_list):
            for ffj in ff_list[i:]:
                eps_mix = sqrt(ffi[2] * ffj[2])
                if params['mixing_rule'].lower() == 'lorentz-berthelot':
                    sig_mix = 0.5 * (ffi[3] + ffj[3])
                elif params['mixing_rule'].lower() == 'jorgensen':
...
def render_molecule_def(ff_data, params, molecule_name):
    ...
    output.append(ff_dict["critical_constants"]["tc"])
    output.append(ff_dict["critical_constants"]["pc"])
    output.append(ff_dict["critical_constants"]["af"])
...
def ff_builder(params):
    ...
    PARAMS_EXAMPLE = Dict( dict = {
        ...
        'tailcorrections': False,           # If True apply tail corrections based on homogeneous-liquid assumption
